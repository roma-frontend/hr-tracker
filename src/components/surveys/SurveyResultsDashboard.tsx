                  ...
                  .map(([val, count]) => (
                    <div key={val} className="flex flex-col items-center gap-1">
                      <div
                        ...
                        style={{ height: `${Math.max(4, (count / Math.max(1, result.totalResponses)) * 60)}px` }}
                      />
      ...
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between gap-2">
          ...
          <Badge variant="secondary" className="text-xs shrink-0">
            {t(`surveys.questionType.${question.type.replace(/_([a-z])/g, (_, c) => c.toUpperCase())}`)}
          </Badge>
        </div>
...
function ExportButton({ surveyId, organizationId }: { surveyId: Id<'surveys'>; organizationId: Id<'organizations'> }) {
  const { t } = useTranslation();
  ...
  return (
    <Button onClick={handleExport} disabled={!exportData || isExporting} variant="outline" size="sm">
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {survey.isAnonymous && (
                <Badge variant="outline">{t('surveys.anonymous')}</Badge>
              )}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {survey.createdAt ? format(new Date(survey.createdAt), 'MMM d', { locale: dateLocale }) : '—'}
                </p>
      ...
      <Tabs defaultValue="overview">
        <TabsList>
          ...
          {!survey.isAnonymous && <TabsTrigger value="responses">{t('surveys.responses')}</TabsTrigger>}
        </TabsList>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Avatar className="h-8 w-8">
                        <AvatarFallback>
                          {resp.respondent?.name?.charAt(0) ?? '?'}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="font-medium text-sm">{resp.respondent?.name ?? t('surveyResults.noData')}</p>
                    ...
                    <span className="text-xs text-muted-foreground">
                      {resp.submittedAt
                        ? format(new Date(resp.submittedAt), 'MMM d, yyyy HH:mm', { locale: dateLocale })
                        : '—'}
                    </span>
                  </div>
                </CardHeader>
                    ...
                    {resp.answers.map((answer) => {
                      ...
                      return (
                        <div key={answer._id} className="flex items-start gap-2 text-sm">
                          ...
                          <span className="text-muted-foreground">
                            {answer.ratingValue ?? answer.textValue ?? (answer.booleanValue !== undefined ? (answer.booleanValue ? 'Yes' : 'No') : answer.selectedOptions?.join(', ') ?? '—')}
                          </span>
                        </div>
                      );
                    })}
