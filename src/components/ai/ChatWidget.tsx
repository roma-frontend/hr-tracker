...
export function ChatWidget() {
  ...
  const [dockedY, setDockedY] = useState(50); // percentage from top
  ...
  // Drag-to-dock: visual position while dragging
    ...
    <>
      ...
      {pathname !== '/ai-chat' && (
        <>
          ...
          {!docked && (
            <motion.div
              className="fixed bottom-16 lg:bottom-6 right-6 z-50 w-10 sm:w-14 h-10 sm:h-14 rounded-full bg-primary/20"
              animate={{
          ...
          {!docked && (
            <motion.button
              ...
              className="fixed bottom-16 lg:bottom-6 right-6 z-50 w-10 sm:w-14 h-10 sm:h-14 rounded-full flex items-center gap-2 justify-center btn-gradient text-white font-medium shadow-md hover:shadow-lg"
              style={
                dragPos
                  ? {
                      position: 'fixed',
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-24 right-2 sm:right-6 z-50 w-[calc(100vw-1rem)] sm:w-[380px] max-h-[calc(100vh-8rem)] flex flex-col rounded-2xl border border-(--border) shadow-2xl overflow-hidden"
