...
export function LeaveRequestModal({ open, onClose }: LeaveRequestModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('[LeaveRequest] Submit clicked', {
      selectedUserId,
      leaveType,
      startDate,
      endDate,
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto px-4 sm:px-6">
        <DialogHeader>
          <DialogTitle className="text-[var(--text-primary)] text-lg sm:text-xl">
            New Leave Request
          </DialogTitle>
          ...
          <form id="leave-request-form" onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="employee" className="text-sm">
                {t('labels.employee')} {t('forms.required')}
              </Label>
              {user?.role !== 'admin' && user?.role !== 'superadmin' ? (
                ...
                <div className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--background-subtle)] text-xs sm:text-sm text-[var(--text-primary)]">
                  {user?.name}{' '}
                  {user?.department && (
                    <span className="text-xs text-[var(--text-muted)]">· {user?.department}</span>
                  )}
                </div>
              ) : (
            ...
            <div className="space-y-1.5">
              <Label className="text-sm">
                {t('labels.leaveType')} {t('forms.required')}
              </Label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-sm">
                  {t('labels.startDate')} {t('forms.required')}
                </Label>
                <Popover modal={true}>
                    ...
                    >
                      ...
                      <span className="truncate">
                        {startDate
                          ...
                          : t('leaveRequest.pickDate')}
                      </span>
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent
                    className="w-auto p-0 max-w-[320px] sm:max-w-none"
                    align="center"
                    side="bottom"
              ...
              <div className="space-y-1.5">
                <Label className="text-sm">
                  {t('labels.endDate')} {t('forms.required')}
                </Label>
                <Popover modal={true}>
                  <PopoverTrigger asChild>
                    <Button
            ...
            <div className="space-y-1.5">
              <Label htmlFor="comment" className="text-sm">
                {t('leaveRequest.additionalComments')}
              </Label>
              <textarea
                id="comment"
                placeholder={t('leaveRequest.commentsPlaceholder')}
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                rows={3}
                className="flex w-full rounded-lg border border-[var(--input-border)] bg-[var(--input)] px-3 py-2 text-xs sm:text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[#2563eb] focus:border-[#2563eb] resize-none"
              />
            </div>
        ...
        <DialogFooter className="gap-2 pt-2 flex-col sm:flex-row">
          ...
          <Button
            ...
            className="w-full sm:w-auto text-sm"
            onClick={(e) => {
              console.log('[LeaveRequest] Submit button clicked!', {
                submitting,
                selectedUserId,
                leaveType,
                startDate,
