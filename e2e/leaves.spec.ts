...
test.describe('Leave Request Flow', () => {
  ...
  test('should display leave request form or button', async ({ page }) => {
    ...
    const hasButton = await page.locator('text=request, text=Request, text=new leave, text=create').count() > 0;
  ...
  test('should show leave balance information', async ({ page }) => {
    ...
    const hasBalance = await page.locator('text=balance, text=remaining, text=available, text=days left').count() > 0;
  ...
  test('leave request form should have required fields', async ({ page }) => {
    ...
    const hasStartDate = await page.locator('input[type="date"], text=start, text=from').count() > 0;
  ...
  test('should show leave request status badges', async ({ page }) => {
    const hasStatuses = await page.locator('text=pending, text=approved, text=rejected, text=cancelled').count() > 0;
  ...
  test('leave types should be selectable', async ({ page }) => {
    ...
    const hasTypes = await page.locator('select, text=vacation, text=sick, text=personal, text=annual').count() > 0;
  ...
  test('date picker should prevent end date before start date', async ({ page }) => {
    ...
    if (await startDateInput.isVisible() && await endDateInput.isVisible()) {
      ...
      if (await submitBtn.isVisible()) {
        ...
        const hasError = await page.locator('text=invalid, text=error, text=must be after').count() > 0;
