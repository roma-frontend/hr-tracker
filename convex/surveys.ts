...
export const getSurveyTrends = query({
  ...
  handler: async (ctx, { organizationId, months = 6 }) => {
    ...
    const avgResponseRate =
      trends.length > 0 ? totalResponses / trends.length : 0;
...
export const getSurveyResponses = query({
  ...
  handler: async (ctx, { surveyId, organizationId, limit = 50 }) => {
    const survey = await ctx.db.get(surveyId);
    if (!survey || survey.organizationId !== organizationId) return null;
    if (survey.isAnonymous) return { error: 'Anonymous survey - individual responses not available' };
    ...
    const responseDetails = await Promise.all(
      responses.map(async (resp) => {
        ...
        return {
          responseId: resp._id,
          respondent: user ? { name: user.name, email: user.email, department: user.department } : null,
      ...
      responses.map(async (resp) => {
        ...
        answers.forEach((ans) => {
          ...
          if (question) {
            ...
            else if (ans.booleanValue !== undefined) answerMap[question.text] = ans.booleanValue ? 'Yes' : 'No';
            else if (ans.selectedOptions?.length) answerMap[question.text] = ans.selectedOptions.join('; ');
          }
        });
